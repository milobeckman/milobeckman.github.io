...
function loadPage() {
    ...
    timerElement.innerText = 'Doubling Blitz'
...
function updateQuestion() {
    questionElement.innerText = answerString(currentScore) + " × 2 = "
}
...
function updateHighScore() {
    try {
        highScore = parseFloat(localStorage.highScorePowersOfTwo)
        if (isNaN(highScore)) {
            highScore = 0
        }
    } catch (e) {
    ...
    if (currentScore > highScore) {
        ...
        localStorage.highScorePowersOfTwo = '' + currentScore
