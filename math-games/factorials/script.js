...
function loadPage() {
    ...
    timerElement.innerText = 'Factorials Blitz'
...
function updateQuestion() {
    questionElement.innerText = answerString(currentScore) + " × " + (currentScore+1) + " = "
}
...
function updateHighScore() {
    try {
        highScore = parseFloat(localStorage.highScoreFactorials)
        if (isNaN(highScore)) {
            highScore = 0
        }
    } catch (e) {
    ...
    if (currentScore > highScore) {
        ...
        localStorage.highScoreFactorials = '' + currentScore
